//
//  RXOtherOperatorViewController.swift
//  RXSwiftDemo
//
...
class RXOtherOperatorViewController: BaseViewController {
    ...
    func materialize() -> Void {
        ...
        Observable.of(1, 2, 3).materialize().subscribe(onNext:{print($0)}).disposed(by: disposeBag)
    }
    ...
    */
    ...
    func timerOut() -> Void {
        ...
        Observable.from(times).flatMap { (item) in
            ...
            }.timeout(DispatchTimeInterval.seconds(2), scheduler: MainScheduler.instance).subscribe(onNext:{element in
